...
class LoginGUI():
    ...
    def make_root(self):
        ...
        self.root.iconbitmap(default= interface['icon']['login'])
        #  icon['login'])
        self.root.geometry(geometry_root(self.root,300,350))
        self.root.resizable(False, False)
        self.root.config(background=interface['palette']['cor5'])
    
    def widhts(self):
        self.label = Label(self.root, text='Usuário')
        self.label.config(
            font=(interface['fon']['1'], 14),
            background=interface['palette']['cor5'],
            anchor='nw'
        )
...
#             font=(fonts[0], 14), justify='center',foreground=palette['cor5'],
